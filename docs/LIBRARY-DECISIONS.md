...
| **License** | Apache-2.0 |
| **Added to** | `DailyDesk.Broker.csproj` |
...
using var db = new LiteDatabase(Path.Combine(stateRoot, "office.db"));
...
**What it introduces:**
- `OfficeJob` record persisted in LiteDB `jobs` collection.
- `OfficeJobWorker : BackgroundService` that processes queued jobs.
